...
def Likelihoodww(data1: np.ndarray, data2: np.ndarray, cv: np.ndarray):
    ...
    data2
        numpy array of headings.
    cv:
        estimated constant airspeed.
    ...
    def f(par):
        ...
        for i, j in zip(data1, data2):
            r1 = np.sqrt(
                pow((i * np.cos(j) - par[3]), 2) + pow((i * np.sin(j) - par[4]), 2)
            )
...
def Weibull_sd(a, b):
    ...
    ----------
    a
        mean heading vector length b:  the shape parameter
    ...
    return b * np.sqrt(
        sp.special.gamma(1 + 2 / a)
        - sp.special.gamma(1 + 1 / a) * sp.special.gamma(1 + 1 / a)
    )
...
def Weibull_mean(a, b):
    ...
    ----------
    a
        mean heading vector direction b:  shape parameter
...
def Von_Mises_sd(kappa) -> np.ndarray:
...
def nearest(items, pivot):
    ...
    ----------
    items
        array to be searched within.
    pivot
        value to find the nearest within array items.
...
def nearestInd(items, pivot):
    ...
    ----------
        ...
        items:          array to be searched within pivot:          value to
        find the index of the nearest value within items
...
def timeRescale(dat, tdiff, units="min"):
    ...
    out = (
        dat.set_index("DT", drop=False).resample(str(tdiff) + units).nearest().dropna()
    )
...
def angles(longitudes, latitudes):
    ...
    ----------
        ...
        longitudes:     narray of longitudes in decimal degrees latitudes:
        array of latitudes in decimal degrees

    Returns
    -------
        Array of length len(longitudes) - 1 showing angles between consecutive
        GPS positions in radians
...
def gps_speed(longitudes, latitudes, timestamps):
    # taken from
    # https://www.tjansson.dk/2021/03/vectorized-gps-distance-speed-calculation-for-pandas/,
    # thanks to Thomas Jansson for use of this function
    """
    Calculates the instantaneous speed from the GPS positions and timestamps.
    The distances between the points are calculated using a vectorized haversine
    calculation the great circle distance between two arrays of points on the
    earth (specified in decimal degrees). All args must be of equal length.

    Parameters
    ----------
        longitudes: pandas series of longitudes latitudes:  pandas series of
        latitudes timestamps: pandas series of timestamps
...
def distSpeed(lat, lon, DT, threshold=None):
    ...
    ----------
        ...
        lat:    array of latitude points in decimal degrees. lon:    array of
        longitude points in decimal degrees. DT:     datetime array in Pandas
        datetime format.
...
def dtFormat(x):
    """
    Format incoming datetimes from BiP system. Typically datetimes are brought
    in the format YYYY-mm-dd HH:MM:SS+00:00, however, SS can contain decimal
    values or not. This function returns the same datetime in string format with
    a decimal place added if none is originally present and removes '+00:' from
    the string so that datetimes can be converted to datetime format in Pandas.
...
def prePare(
    filename, convertToMin: bool = True, tdiff=1, units="min", isBip: bool = True
):
    """Prepare AxyTrek GPS data as per required for Goto original method.
    DateTime, distance, track speed and direction is added

    Parameters
    ----------
        filename:       full path for file to be read in convertToMin:   boolean
        for whether data should be resampled to regular time intervals tdiff:
        resample time interval. Defaults to 1 units:          resample time
        interval unit. Defaults to 'min' isBip:          boolean stating whether
        data is in BiP format

    Returns
    -------
        Pandas dataframe of read in data. Adds columns 'dt' (elapsed time from
        fix to previous time point in seconds), 'dist' (distance travelled from
        previous point in m), 'track_speed' (in m/sec), 'track_direction'
        (direction between consecutive GPS positions in rad)
    """
    ...
    df["dt"] = np.append(
        np.nan, (np.diff(df["DT"]) / np.timedelta64(1, "s")).astype(float)
    )
    df["dist"], df["track_speed"] = gps_speed(df["lon"], df["lat"], df["DT"])
    df["track_direction"] = angles(df["lon"], df["lat"])
    return df.dropna().reset_index()[
        ["DT", "lon", "lat", "dt", "dist", "track_speed", "track_direction"]
    ]


def A1inv(x):
    """Copy of A1inv function from circular package in R.
    Inverse function of the ratio of the first and zeroth order Bessel functions
    of the first kind. This function is used to compute the maximum likelihood
    estimate of the concentration parameter of a von Mises distribution.
    ...
    Returns
    -------
        Value of x such that A1inv(x) = k and A1(k) = x where A1 is the ratio
        between the first and zeroth order Bessel function of the first kind
    """
    if (0 <= x) & (x < 0.53):
...
def rangeGen(DF, cen, wws, cutV, cutT, cutLength):
    ...
    ----------
        ...
        DF:         dataframe of tag recordings as output by prePare() cen:
        index of the center value to be tested wws:        window width in
        seconds cutV:       minimum speed to ensure flight behaviour cutT:
        maximum time difference between consecutive GPS points in seconds
        cutLength:  minimum number of samples allowed for model window

    Returns
    -------
        If window conforms to model requirements (number of samples, flight
        speed, etc.), the range of indeces of the appropriate window and the
        center index of the window are returned
    """
    st = DF.dt[:cen][::-1].cumsum().gt(wws).idxmax() + 1
    end = DF.dt[(cen + 1) :].cumsum().gt(wws).idxmax() + 1
    if (
        sum(
            (DF.track_speed[st:end] > cutV)
            & (DF.dt[st:end] < cutT)
            & (DF.track_direction[st:end] != 100)
        )
        > cutLength
    ):
...
def findWindows(DF, cutv=4.1667, windowlength=51):
    ...
    Returns
    -------
        Two arrays, a range array of windows that meet the model requirements,
        and an int array of center points of each window
    """
    ...
    error_of_sampling_interval = (
        5 * fs
    )  # give 5 seconds of leeway for samples to be found in
    ...
    windows, centers = zip(
        *filter(
            ...
            [
                rangeGen(DF, center, windwidthsec, cutv, cutt, cutlength)
                for center in range(
                    centr, DF.dt[::-1].cumsum().gt(windwidthsec).idxmax()
                )
            ],
        )
    )
...
def initPars(head, spd, hed, cv=34.7 / 3.6):
    ...
    Parameters
    ----------
        head:   heading direction (this value will range between -3:3 in
        testing) spd:    array of window track speeds hed:    array of window
        track headings cv:     an assumed constant air speed (m/s). Defaults to
        34.7/3.6 as per Shiomi et al. 2012

    Returns
    -------
        Mean heading (float), an initial value for kappa, the von Mises
        concentration parameter, and a length 5 array of initial values for
...
def windOptims(spd, hed, cv, pars):
    """Optimisation for log-likelihood function produced by Likelihoodww() using
    Nelder-Mead methods

    Parameters
    ----------
        spd:    array of window track speeds hed:    array of window track
        headings cv:     an assumed constant air speed (m/s). Defaults to
        34.7/3.6 as per Shiomi et al. 2012 pars:   initial parameters to produce
        log-likelihood function
    ...
    """
    return minimize(
        Likelihoodww(spd, hed, cv),
        pars,
        bounds=([0.01, None], [None, None], [None, None], [None, None], [None, None]),
        method="L-BFGS-B",
    )


def yokoTate(optimAns, cv):
    """Calculates von Mises SD and mean product along with Weibull standard
    deviation

    Parameters
    ----------
        optimAns:   output of windOptims for calculation of von Mises and
        Weibull characteristics cv:         an assumed constant air speed (m/s).
        Defaults to 34.7/3.6 as per Shiomi et al. 2012

    Returns
    -------
        Product of von Mises SD and mean and the Weibull standard deviation,
        both using optimised values
    """
    yoko = Von_Mises_sd(
        np.sqrt(optimAns[2] * optimAns[2] + optimAns[1] * optimAns[1])
    ) * Weibull_mean(optimAns[0], cv / sp.special.gamma(1 + 1 / optimAns[0]))
...
def ensureOptimConv(optimAns, spd, hed, cv):
    ...
    ----------
        ...
        spd:        window speed values (m/s) hed:        window heading values
        (rad) cv:         an assumed constant air speed (m/s). Defaults to
        34.7/3.6 as per Shiomi et al. 2012

    Returns
    -------
        Converged maximum likelihood estimates, along with their outputs of
        yokoTate()
    """
...
def windOptim(initHead, spd, head, cv):
    ...
    Parameters
    ----------
        initHead:   initial value for heading spd:        window track speeds
        (m/s) head:       window directions (rad) cv:         an assumed
        constant air speed (m/s). Defaults to 34.7/3.6 as per Shiomi et al. 2012

    Returns
    -------
        If MLE has converged and values pass the required tests, optimised
        values and resultant von Mises and Weibull characteristics
    """
...
def headSpdDir(spd, hed, answ):
    ...
    Parameters
    ----------
        spd:    window track speeds (m/s) hed:    window track headings (rad)
...
def GOFtests(hed, nr, nd, answ, yoko, tate, cv):
    ...
    Parameters
    ----------
        hed:    window track headings (rad) nr:     heading vector speed nd:
        heading vector direction answ:   optimised MLE estimates yoko:   von
        Mises characteristics tate:   Weibull characteristics cv:     an assumed
        constant air speed (m/s). Defaults to 34.7/3.6 as per Shiomi et al. 2012
    ...
    nrp = sp.stats.kstest(
        nr,
        partial(
            sp.stats.weibull_min.cdf,
            c=answ.x[0],
            scale=cv / sp.special.gamma(1 + 1 / answ.x[0]),
        ),
    )
...
def maxLikeWind(r, d, cv):
    """Generate optimal MLE estimation and return estimates if all three
    conditions are met

    Parameters
    ----------

        r:  track speeds (m/s) d:  track headings (rad) cv: an assumed constant
        air speed (m/s). Defaults to 34.7/3.6 as per Shiomi et al. 2012
    ...
    for id_hd in range(-hd_try, hd_try):
        ...
        if hasattr(answ, "success"):
            if (not np.isnan(tate)) & (answ.status == 0):
                ...
                # GOF tests
                if np.isnan(max_like) & (
                    np.prod(GOFtests(d, nr, nd, answ, yoko, tate, cv)) == 1
                ):
                    max_like = answ.fun
                    answ_best = answ

                if (answ.fun > max_like) & (
                    np.prod(GOFtests(d, nr, nd, answ, yoko, tate, cv)) == 1
                ):
                    max_like = answ.fun
                    answ_best = answ
...
def windEstimation(
    ...
    ----------
        ...
        cutv:           minimum ground speed in m/s (default: 4.1667) cv:
        mean air speed in m/s (default: 34.7 kph) windowLength:   window size
        for estimation in mins (default: 51) rescaleTime:    should original
        data be rescaled to 1 fix/min (default: True) isBp:           boolean,
        is data formatted by BiP (T) or X Manager (F)

    Returns
    -------
        Pandas dataframe of wind estimation methods. Columns are: centerpoint
        datetime ('Time'), lat ('Lat'), lon ('Lon'), mean bird heading during
        window (rad, east is 0. 'MeanHead'), wind vector x ('X'), wind vector y
        ('Y')
    """

    # # write the header of the outfile
    # with open(outfile,'w',newline='') as f: writer = csv.writer(f, delimiter =
        ...
        if type(answ_best) != float:
            out.append(
                [
                    ...
                    dat.lat[centers[win]],
                    dat.lon[centers[win]],
                    np.arctan2(answ_best.x[2], answ_best.x[1]),
                    answ_best.x[3],
                    answ_best.x[4],
                ]
            )
...
def roundTo(x, rn):
    return np.format_float_positional(
        x, precision=rn, unique=False, fractional=False, trim="k"
    )
...
# taken from 'https://gist.github.com/kn1cht/89dc4f877a90ab3de4ddef84ad91124e',
# circular python package by kn1cht
...
def windEstimation2(
    ...
    """Wind estimation from bird GPS positions.

    Parameters
    ----------
    filename
        location of AxyTrek data (BiP or X Manager formatted).
    cutv
        minimum ground speed in m/s (default: 4.1667).
    cv
        mean air speed in m/s (default: 34.7 kph).
    windowLength
        window size for estimation in mins (default: 51).
    rescaleTime
        should original data be rescaled to 1 fix/min (default: True).
    isBp
        is data formatted by BiP (T) or X Manager (F).
...
def removeNear(acc_data, gps_data, captureSite, distThreshold=1.5):
    ...
    fs = int(
        np.timedelta64(1, "s") / stats.mode(np.diff(acc_data.DT), keepdims=False)[0]
    )

    # GPS sample rate (fixes per minute)
    gpsFS = 60 / int(
        np.timedelta64(1, "m") / stats.mode(np.diff(gps_data["DT"]), keepdims=False)[0]
    )

    # find distances from capture site
    capSiteDist = np.array(
        gps_distanceSingle(gps_data.lon, gps_data.lat, captureSite[0], captureSite[1])
    )

    inds = gps_data.lat.index[
        np.where(capSiteDist < distThreshold)[0]
    ]  # find indices to be removed from original dataset

    # if any difference in GPS and acc sampling rates means we must extend GPS indices
    samplerDiff = int(
        np.floor(fs * gpsFS / 2)
    )  # number of fs samples either side of GPS position
...
def main_func(df):
    ...
    df.rename(
        columns={"time": "DT", "latitude": "lat", "longitude": "lon"}, inplace=True
    )
    df["DT"] = [dtFormat(str(x)) for x in df["DT"]]
    df["DT"] = pd.to_datetime(
        df["DT"], format="%Y-%m-%d %H:%M:%S.%f"
    )  # 一旦、+00:00を除くために文字列にする

    if len(df) > 0:
        out = windEstimation2(df, isBp=True)
        out = out.rename(
            columns={"Time": "time", "Lat": "latitude", "Lon": "longitude"}
        )

        # すでにUTCなので不要
        out["time"] = pd.to_datetime(
            out["time"], format="%Y-%m-%d %H:%M:%S.%f", errors="coerce"
        ).dt.tz_localize("UTC")
