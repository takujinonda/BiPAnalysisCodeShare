...
def Likelihoodww(data1: np.ndarray, data2: np.ndarray, cv: np.ndarray):
    ...
    Args:
        data1:  numpy array of speed values data2:  numpy array of headings cv:
...
def Weibull_sd(a, b):
    ...
    Args:
        ...
        a:  mean heading vector length b:  the shape parameter
...
def Weibull_mean(a, b):
    ...
    Args:
        ...
        a:  mean heading vector direction b:  shape parameter
...
def readAxyGPS(
    ...
    Args:
        ...
        filename:   path to AxyTrek txt file cols:       list of columns to be
        read in, defaults to [0,1,2,3] colnames:   list of string column names
        to be assigned. Must be of same length as cols. Defaults to ['Date',
        'Time', 'lat', 'lon']

    Returns:
        Pandas dataframe of columns `colnames`. A formatted DateTime column
        (named DT) is generated.
    """
    df = pd.read_csv(filename, sep=delim, usecols=cols, names=colnames)
...
def readBIPAxy(filename):
    ...
    Returns:
        Pandas dataframe of all columns from the BIP system (datetime 'DT',
        latitude 'lat', and longitude 'lon'). A formatted DateTime column (named
        DT) is generated)
    """

    df = pd.read_csv(filename, sep=",", header=0).dropna().reset_index()
    df = df[["time", "latitude", "longitude"]].rename(
...
def nearest(items, pivot):
    ...
    Args:
        ...
        items:  array to be searched within pivot:  value to find the nearest
        within array items
...
def timeRescale(dat, tdiff, units="min"):
    ...
    values are used, `dat` must have datetime column 'DT' Args:
